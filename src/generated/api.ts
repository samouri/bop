...
export class BaseAPI {
  ...
  constructor(
    fetch: FetchAPI = isomorphicFetch as any,
...
export const DefaultApiFp = {
  ...
  rootGet(options: any = {}): (fetch: FetchAPI, basePath?: string) => Promise<any> {
    ...
    return (fetch: FetchAPI = (isomorphicFetch as any) as any, basePath: string = BASE_PATH) => {
...
export const MetadataApiFp = {
  ...
  metadataDelete(
    ...
    return (fetch: FetchAPI = isomorphicFetch as any, basePath: string = BASE_PATH) => {
      return fetch(basePath + fetchArgs.url, fetchArgs.options).then((response) => {
        if (response.status >= 200 && response.status < 300) {
          return response
        } else {
