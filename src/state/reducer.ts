...
export const getCurrentUser = (state): ApiUser =>
  state.users.byId[state.currentUser] || {};
...
export const getAllSongsDenormalized = createSelector(
  [getSongEntities, getState],
  (songs, state) => _.map(songs, (song) => getDenormalizedSong(state, song))
);
...
export const getDenormalizedSong = createSelector(
  [getState, getProps],
  (state, { id }) => {
    ...
    return {
      ...
      votes: _.filter(
        state.votes.byId,
        (vote: ApiVotes) => vote.song_id === song.id
      ),
    };
  }
);
...
export const sortSongs = (
  denormalizedSongs: Array<DenormalizedSong>,
  sort: SORT
) => {
  switch (sort) {
    case "votes":
      return _.reverse(
        _.sortBy(denormalizedSongs, (song) => song.votes.length)
      );
    case "date":
...
export const getSongsInStream = createSelector(
  ...
  (
    ...
    if (type === "playlist") {
      ...
      songs = _.filter(
        denormalizedSongs,
        (song) => !!_.find(song.votes, { user_added: id })
      );
    }
...
export const getContributorsInPlaylist = createSelector(
  ...
  (state, playlistId, songs: Array<DenormalizedSong>, usersById) => {
    ...
    const sortedContribs = _.reverse(
      _.uniq(_.sortBy(contribs, (c: string) => counts[c]))
    );
...
const playlistsById = handleActions(
  {
    [combineActions(ADD_ENTITIES, SET_PLAYLIST) as any]: (
      state,
      action: any
    ) => {
...
export const getCombinedEvents = createSelector(
  [getEventsDenormalized],
  (events) => {
    ...
    _.forEach(events, (event, i: number) => {
      ...
      if (
        ...
        lastPlaylist === _.get(event, ["song", "playlist_id"])
      ) {
...
const playerShuffle = handleActions(
  { [SHUFFLE_SONGS]: (state) => !state },
  false
);
const playerPlaying = handleActions(
  { [PLAY_SONG]: () => true, [PAUSE_SONG]: () => false },
  false
);
...
const currentUser = handleActions(
  {
    [LOGIN_USER]: (state, action: any) =>
      action.error ? null : action.payload.id,
    [LOGOUT_USER]: () => null,
  },
  null
);
