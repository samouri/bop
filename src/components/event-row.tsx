...
const VoteEvent = ({ event }) => {
  ...
  return (
    <div>
      <Link to={`/u/${user && user.username}`}>@{user && user.username}</Link>{" "}
      upvoted a song on <Link to={`/p/${playlistName}`}>{playlistName}</Link>
    </div>
  );
};
...
class SingleEvent extends React.Component<Props> {
  ...
  durationToString() {
    var duration = moment.duration(
      this.props.event.song.metadata.youtube_duration
    );
...
const EventRow = withPlayer(({ event, stream, player }) => {
  ...
  return (
    <ConnectedSingleEvent
      event={event}
      stream={stream}
      songId={event.song && event.song.id}
    />
  );
});
